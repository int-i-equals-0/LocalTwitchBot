...
function bufferReward(rewardId, username, userMessage, eventId = null) {
    ...
    console.log(`[INFO] Награда ${rewardId} от ${username} добавлена в буфер (${rewardBuffer.length} в очереди)`);
    
    // Очищаем старые записи, если буфер слишком большой
    if (rewardBuffer.length > EVENTSUB_SETTINGS.MAX_BUFFER_SIZE) {
        const removed = rewardBuffer.splice(0, rewardBuffer.length - EVENTSUB_SETTINGS.MAX_BUFFER_SIZE);
        console.log(`[INFO] Буфер наград очищен: удалено ${removed.length} старых записей`);
    }
}
...
async function processRewardBuffer() {
    ...
    console.log(`[INFO] Обрабатываем буфер наград (${rewardBuffer.length} шт)...`);
    ...
    for (const reward of bufferCopy) {
        ...
        if (Date.now() - reward.timestamp < EVENTSUB_SETTINGS.BUFFER_TIMEOUT) {
            ...
            console.log(`[INFO] Награда ${reward.rewardId} от ${reward.username} слишком старая (${Math.floor((Date.now() - reward.timestamp) / 1000)} сек), пропускаем`);
            skippedCount++;
        }
    }
    
    console.log(`[INFO] Буфер обработан: ${processedCount} обработано, ${skippedCount} пропущено`);
}
...
function sendLogToClients(msg) {
    ...
    logClients.forEach(c => { if (c.readyState === WebSocket.OPEN) c.send(d); });
}
...
function registerStartupTask(task) { startupTasks.add(task); }
function completeStartupTask(task) {
    completedTasks.add(task);
    if (!botFullyReady && startupTasks.size > 0 && [...startupTasks].every(t => completedTasks.has(t))) {
        botFullyReady = true;
        console.log('[START] ========================================');
        if (tokensValid) {
...
function updateFromConfig() {
    if (FULL_CONFIG.tokens) {
        if (FULL_CONFIG.tokens.channelName) CONFIG.channel = FULL_CONFIG.tokens.channelName;
        if (FULL_CONFIG.tokens.botUsername) CONFIG.botUsername = FULL_CONFIG.tokens.botUsername;
        if (FULL_CONFIG.tokens.clientId) CONFIG.clientId = FULL_CONFIG.tokens.clientId;
        if (FULL_CONFIG.tokens.clientSecret) CLIENT_SECRET = FULL_CONFIG.tokens.clientSecret;
        ...
        if (FULL_CONFIG.tokens.broadcasterAccessToken) {
            BROADCASTER_TOKEN = FULL_CONFIG.tokens.broadcasterAccessToken.replace('oauth:', '');
        }
...
function hasRequiredTokens() {
    ...
    if (!tokens.broadcasterAccessToken) missing.push('broadcasterAccessToken (токен стримера)');
...
async function refreshTwitchToken(type) {
    ...
    try {
        ...
        if (type === 'bot') {
            ...
            if (data.refresh_token) tokens.broadcasterRefreshToken = data.refresh_token;
        ...
        console.error(`[ERROR] Ошибка при обновлении токена ${type}:`, error.message);
        return false;
    }
}
...
async function scheduleTokenRefresh() {
    ...
    if (OAUTH_TOKEN) {
        ...
        if (!valid) {
            ...
            console.log(`[INFO] Токен бота истекает через ${Math.floor(expiresIn / 60)} мин. Обновление через ${Math.floor(refreshIn / 60000)} мин.`);
    ...
    if (BROADCASTER_TOKEN) {
        ...
        if (!valid) {
            ...
            const refreshIn = Math.max(expiresIn - 300, 60) * 1000;
            console.log(`[INFO] Токен стримера истекает через ${Math.floor(expiresIn / 60)} мин. Обновление через ${Math.floor(refreshIn / 60000)} мин.`);
...
console.log(`[START] Вебсокет для оверлеев: ws://${getLocalIP()}:${CONFIG.wsPort}`);

const connectedOverlays = new Map();

wss.on('headers', (headers) => { headers.push('Access-Control-Allow-Origin: *'); });
...
wss.on('connection', (ws, req) => {
    ...
    console.log(`[INFO] Оверлей подключился: id=${overlayId}, path=${overlayPath}`);

    ws.on('message', (message) => {
        try {
            const data = JSON.parse(message);
            if (data.command === 'register') {
                connectedOverlays.set(ws, { overlayId: data.overlayId || overlayId, overlayPath: data.overlayPath || overlayPath });
            }
        } catch (e) {}
    });
    ...
    ws.on('close', () => {
        ...
        console.log(`[INFO] Оверлей отключился: id=${info?.overlayId || 'unknown'}`);
    });
});
...
function notifyOverlay(command, data, targetOverlay) {
    ...
    if (targetOverlay && targetOverlay.trim() !== '') {
        ...
        const config = loadFullConfig();
        const overlayConfig = (config.overlays || []).find(o =>
            o.id === target || o.path === target || o.name === target
        );
        ...
        if (!sent) {
            console.warn(`[WARN] Оверлей ${getOverlayUrl(target)} не подключен. Событие отброшено.`);
        }
        return sent;
    }
...
function getOverlayUrl(overlayTarget) {
    ...
    const config = loadFullConfig();
    const overlay = (config.overlays || []).find(o =>
        o.id === overlayTarget || o.path === overlayTarget || o.name === overlayTarget
    );
    if (overlay) {
...
async function fetchViewersFromAPI() {
    ...
    try {
        ...
        if (!chatResponse.ok) return await fetchViewersViaOldAPI(broadcasterId, accessToken);
        const chatData = await chatResponse.json();
        if (chatData.data?.length > 0) return chatData.data.map(c => c.user_name).filter(n => n.toLowerCase() !== BOT_USERNAME.toLowerCase());
        return [];
    } catch (error) {
...
async function fetchViewersViaOldAPI(broadcasterId, accessToken) {
    try {
        const [modsResp, vipsResp] = await Promise.all([
            fetch(`https://api.twitch.tv/helix/moderation/moderators?broadcaster_id=${broadcasterId}&first=100`, { headers: { 'Client-Id': CLIENT_ID, 'Authorization': `Bearer ${accessToken}` } }),
            fetch(`https://api.twitch.tv/helix/channels/vips?broadcaster_id=${broadcasterId}&first=100`, { headers: { 'Client-Id': CLIENT_ID, 'Authorization': `Bearer ${accessToken}` } })
        ]);
        const mods = await modsResp.json();
        const vips = await vipsResp.json();
        return [...new Set([...(mods.data?.map(m => m.user_name) || []), ...(vips.data?.map(v => v.user_name) || [])])];
    } catch (error) { return []; }
}
...
async function getRandomViewer() {
    try {
        ...
        if (viewersCache.length > 0) return viewersCache[Math.floor(Math.random() * viewersCache.length)];
        if (recentChatters.size > 0) { const arr = Array.from(recentChatters); return arr[Math.floor(Math.random() * arr.length)]; }
        const fb = ['friend', 'viewer', 'chatter', 'follower', 'subscriber'];
        return fb[Math.floor(Math.random() * fb.length)];
    } catch (error) { return 'someone'; }
}

if (tokensValid) {
    setTimeout(() => scheduleTokenRefresh(), 5000);
    setInterval(async () => { try { viewersCache = await fetchViewersFromAPI(); viewersCacheTime = Date.now(); } catch (e) {} }, VIEWERS_CACHE_TTL);
}

// ========== ЗАГРУЗКА ДАННЫХ ==========
async function loadCommands() { try { return loadFullConfig().commands || {}; } catch (e) { return {}; } }
function loadRewards() { try { return loadFullConfig().rewards || {}; } catch (e) { return {}; } }
async function loadBanWords() { try { return loadFullConfig().banwords || { words: [] }; } catch (e) { return { words: [] }; } }
function loadEvents() { try { return loadFullConfig().events || {}; } catch (e) { return {}; } }
function loadAutoShoutout() { try { return loadFullConfig().autoshoutout || []; } catch (e) { return []; } }
function loadPeriodicEvents() { try { return loadFullConfig().periodicEvents || {}; } catch (e) { return {}; } }

// ========== БАНВОРДЫ ==========
function generateAliases(word) {
    if (!word) return [];
    const CHAR_MAP = { 'а':['a'],'б':['6'],'в':['b'],'г':['r'],'е':['e'],'к':['k'],'м':['m'],'н':['h'],'о':['o'],'р':['p'],'с':['c'],'т':['t'],'у':['y'],'х':['x'],'ь':['b'],'a':['а'],'b':['в','ь'],'c':['с'],'e':['е'],'h':['н'],'k':['к'],'m':['м'],'o':['о'],'p':['р'],'t':['т'],'r':['г'],'x':['х'],'y':['у'] };
    const original = word.toLowerCase();
    const aliases = new Set([original]);
    const chars = original.split('');
    const positions = [];
    chars.forEach((char, i) => { if (CHAR_MAP[char]) positions.push({ index: i, replacements: CHAR_MAP[char] }); });
    ...
    function gen(current, pos) {
        ...
        gen([...current], pos + 1);
        for (const r of p.replacements) { const n = [...current]; n[p.index] = r; gen(n, pos + 1); }
    }
    gen([...chars], 0);
...
function containsBannedWord(message, banWords) {
    if (!message || !banWords || !Array.isArray(banWords)) return { found: false };
    const ml = message.toLowerCase();
    const variants = [...new Set([ml, normalizeMessage(ml), removeRepeatedChars(ml), replaceSimilarChars(ml)])];
    for (const item of banWords) {
        if (!item?.word) continue;
        const words = [item.word.toLowerCase()];
        if (item.aliases?.length) words.push(...item.aliases.map(a => a.toLowerCase()));
        else { const a = generateAliases(item.word); words.push(...a); item.aliases = a; }
        for (const v of variants) {
            for (const w of [...new Set(words)]) {
                if (item.type === 'hard') { if (v.includes(w)) return { found: true, word: item.word, type: 'hard' }; }
                else { if (new RegExp(`(^|[\\s.,!?;:()\\[\\]{}"'_])${w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}([\\s.,!?;:()\\[\\]{}"'_]|$)`,'i').test(v)) return { found: true, word: item.word, type: 'soft' }; }
            }
        }
    }
    return { found: false };
...
function normalizeMessage(m) { const map = {'@':'а','a':'а','b':'в','6':'б','c':'с','e':'е','3':'е','h':'н','x':'х','i':'и','1':'и','o':'о','0':'о','p':'р','y':'у'}; let r=m; for(const[f,t]of Object.entries(map))r=r.replace(new RegExp(f,'g'),t); return r; }
function removeRepeatedChars(m) { return m.replace(/(.)\1+/g, '$1'); }
function replaceSimilarChars(m) { const map={'a':'а','b':'в','c':'с','e':'е','h':'н','i':'и','k':'к','m':'м','o':'о','p':'р','t':'т','x':'х','y':'у'}; let r=m; for(const[l,c]of Object.entries(map))r=r.replace(new RegExp(l,'g'),c); return r; }

async function logDeletedMessage(username, message, word, type) {
    try { await fsPromises.appendFile(path.join(__dirname, 'deleted_messages.log'), `[${new Date().toISOString()}] Удалено от ${username}: "${message}" (слово: "${word}", тип: ${type})\n`); } catch (e) {}
}
...
async function checkBotPermissions() {
    try {
        ...
        const v = await fetch('https://id.twitch.tv/oauth2/validate', { headers: { 'Authorization': `Bearer ${OAUTH_TOKEN}` } });
        if (v.ok) {
            ...
            const requiredBotScopes = [
                ...
                'channel:moderate', 'moderator:manage:chat_messages', 'moderator:manage:shoutouts'
            ];
            ...
            const missingScopes = requiredBotScopes.filter(scope => !d.scopes.includes(scope));
            if (missingScopes.length > 0) {
                console.warn(`[WARN] Боту не хватает прав: ${missingScopes.join(', ')}`);
            } else {
        ...
        if (BROADCASTER_TOKEN) {
            ...
            const bv = await fetch('https://id.twitch.tv/oauth2/validate', { headers: { 'Authorization': `Bearer ${BROADCASTER_TOKEN}` } });
            if (bv.ok) {
                ...
                const requiredBroadcasterScopes = [
                    ...
                    'channel:read:redemptions', 'moderator:read:shoutouts', 'moderator:manage:shoutouts'
                ];
                ...
                const missingScopes = requiredBroadcasterScopes.filter(scope => !bd.scopes.includes(scope));
                if (missingScopes.length > 0) {
                    console.warn(`[WARN] Стримеру не хватает прав: ${missingScopes.join(', ')}`);
                } else {
...
// ========== СБОРКА СООБЩЕНИЙ ==========
async function buildMessageFromComponents(components, author, target, extraVars) {
    if (!components || !Array.isArray(components)) return '';
    let result = '';
    for (const comp of components) {
        switch (comp.type) {
            case 'author': result += `@${author}`; break;
            case 'target':
                if (!target || target === 'someone') result += `@${await getRandomViewer()}`; else result += target;
                break;
            case 'randomViewer': result += `@${await getRandomViewer()}`; break;
            case 'static':
                let val = comp.value || '';
                if (extraVars) {
            ...
            case 'random': result += Math.floor(Math.random() * ((comp.max||100) - (comp.min||0) + 1)) + (comp.min||0); break;
            case 'phrase':
                if (comp.phrases?.length > 0) { const valid = comp.phrases.filter(p => p.trim()); if (valid.length) result += valid[Math.floor(Math.random() * valid.length)]; }
                break;
            case 'variable':
...
// ========== ВЫПОЛНЕНИЕ ДЕЙСТВИЯ ==========
async function executeAction(config, channel, author, target, extraVars, sourceName) {
    ...
    if (config.response?.chat?.enabled && config.response.chat.components) {
        const response = await buildMessageFromComponents(config.response.chat.components, author, target, extraVars);
        if (response && channel) {
            if (twitchClient) {
    ...
    if (config.response?.media?.enabled && config.response.media.file) {
        ...
        if (sent) {
            ...
            console.warn(`[WARN] Медиа НЕ доставлено: "${label}" -> ${overlayUrl} (оверлей не подключен)`);
        }
    }
...
async function handleReward(rewardId, username, userMessage) {
    ...
    if (lastHandled && (now - lastHandled) < 10000) {
        console.log(`[INFO] Награда ${rewardId} от ${username} — дубликат, пропуск`);
        return true;
    }
    ...
    for (const [key, config] of Object.entries(rewards)) {
        if (config.rewardId === rewardId) { rewardConfig = config; break; }
    }
    ...
    const result = await executeAction(rewardConfig, `#${CHANNEL_NAME}`, username, userMessage || undefined, extraVars, rewardConfig.rewardTitle);

    if (result === false) {
        console.warn(`[WARN] Награда "${rewardConfig.rewardTitle}" — медиа не доставлено (оверлей не подключен)`);
    }
...
async function handleRewardWithBuffer(rewardId, username, userMessage, eventId = null) {
    // Если EventSub не готов или переподключается, буферизируем
    if (!eventSubWs || eventSubWs.readyState !== WebSocket.OPEN) {
        console.log(`[INFO] EventSub не готов (state: ${eventSubWs?.readyState || 'null'}), буферизируем награду от ${username}`);
...
async function handleEvent(eventType, eventData) {
    ...
    await executeAction(eventConfig, channel, username, undefined, extraVars, `Событие: ${eventType}`);
}
...
async function getUserIdByName(username) {
    try {
        ...
        const resp = await fetch(`https://api.twitch.tv/helix/users?login=${username.toLowerCase()}`, {
        ...
        console.error(`[ERROR] Ошибка получения ID для ${username}:`, e.message || e);
        return null;
    }
}
...
async function sendShoutout(username) {
    try {
        ...
        if (resp.status === 204) {
            ...
            if (resp.status === 429 || errData?.message?.includes('already been sent')) {
                return false;
            }
            return true;
        }
...
async function processShoutoutQueue() {
    ...
    while (shoutoutQueue.length > 0) {
        ...
        if (now < shoutoutCooldownUntil) {
            ...
            console.log(`[INFO] Shoutout: ожидание ${Math.ceil(waitMs / 1000)} сек до следующего...`);
        ...
        if (success) {
            ...
            console.log(`[INFO] Shoutout для ${username} выполнен. В очереди: ${shoutoutQueue.length}`);
        } else {
            console.warn(`[WARN] Shoutout для ${username} не удался, повтор через 30 сек`);
...
function queueShoutout(username) {
    ...
    const isInList = autoList.some(name => name.toLowerCase() === lowerUsername);
    if (!isInList) return;

    shoutoutDone.add(lowerUsername);

    if (shoutoutQueue.some(name => name.toLowerCase() === lowerUsername)) return;

    shoutoutQueue.push(username);
    console.log(`[INFO] ${username} добавлен в очередь shoutout (позиция: ${shoutoutQueue.length})`);
    ...
    eventSubWs.on('message', async (rawData) => {
        try {
            ...
            if (messageType === 'session_welcome') {
                ...
                const keepaliveSeconds = data.payload.session.keepalive_timeout_seconds || EVENTSUB_SETTINGS.KEEPALIVE_TIMEOUT;

                const t = botFullyReady ? '[INFO]' : '[START]';
                console.log(`${t} EventSub session: ${eventSubSessionId}`);
                console.log(`${t} EventSub keepalive timeout: ${EVENTSUB_SETTINGS.KEEPALIVE_TIMEOUT} сек`);
                ...
                console.log(`[INFO] EventSub: сервер просит переподключиться к ${reconnectUrl}`);
                reconnectEventSub(reconnectUrl);
            } else if (messageType === 'notification') {
                await handleEventSubNotification(data);
            } else if (messageType === 'revocation') {
                const sub = data.payload.subscription;
                console.warn(`[WARN] EventSub: подписка отозвана. Тип: ${sub.type}, причина: ${sub.status}`);
            }
        } catch (error) {
            console.error('[ERROR] EventSub ошибка обработки:', error.message || error);
        }
    });
...
function resetKeepaliveTimeout(keepaliveSeconds) {
    ...
    const timeoutMs = (Math.max(keepaliveSeconds, EVENTSUB_SETTINGS.KEEPALIVE_TIMEOUT) + 5) * 1000;
...
async function handleEventSubNotification(data) {
    ...
    if (subscriptionType === 'channel.channel_points_custom_reward_redemption.add') {
        ...
        console.log(`[INFO] EventSub: Награда "${rewardTitle}" (${rewardId}) от ${username}${userInput ? `: "${userInput}"` : ''}`);
        ...
        await handleEvent('follow', { username: username, userId: event.user_id, user: username });
    } else if (subscriptionType === 'channel.subscribe') {
        const username = event.user_name;
        const tier = event.tier;
        const isGift = event.is_gift;
        const tierName = tier === '1000' ? 'Tier 1' : tier === '2000' ? 'Tier 2' : tier === '3000' ? 'Tier 3' : tier;
        console.log(`[INFO] Новая подписка: ${username} (${tierName}${isGift ? ', подарок' : ''})`);
        await handleEvent('subscribe', { username, userId: event.user_id, user: username, tier: tierName, tierRaw: tier, isGift: isGift ? 'true' : 'false' });
        ...
        const tierName = tier === '1000' ? 'Tier 1' : tier === '2000' ? 'Tier 2' : tier === '3000' ? 'Tier 3' : tier;
        console.log(`[INFO] Переподписка: ${username} (${tierName}, ${months} мес.)`);
        await handleEvent('resubscribe', { username, user: username, tier: tierName, tierRaw: tier, months: String(months), streakMonths: String(streakMonths), message });
    } else if (subscriptionType === 'channel.subscription.gift') {
        const gifterName = event.user_name || 'Аноним';
        const total = event.total;
        const tier = event.tier;
        const tierName = tier === '1000' ? 'Tier 1' : tier === '2000' ? 'Tier 2' : tier === '3000' ? 'Tier 3' : tier;
        console.log(`[INFO] Подарочные подписки: ${gifterName} подарил ${total} x ${tierName}`);
        await handleEvent('giftSub', { username: gifterName, user: gifterName, tier: tierName, tierRaw: tier, total: String(total), isAnonymous: event.is_anonymous ? 'true' : 'false' });
        ...
        await handleEvent('raid', { username: fromUser, user: fromUser, viewers: String(viewers), fromUserId: event.from_broadcaster_user_id });
    }
}
...
async function subscribeToAllEvents() {
    ...
    const subscriptions = [
        { type: 'channel.channel_points_custom_reward_redemption.add', version: '1', condition: { broadcaster_user_id: channelId }, token: broadcasterToken },
        { type: 'channel.follow', version: '2', condition: { broadcaster_user_id: channelId, moderator_user_id: channelId }, token: broadcasterToken },
        { type: 'channel.subscribe', version: '1', condition: { broadcaster_user_id: channelId }, token: broadcasterToken },
        { type: 'channel.subscription.message', version: '1', condition: { broadcaster_user_id: channelId }, token: broadcasterToken },
        { type: 'channel.subscription.gift', version: '1', condition: { broadcaster_user_id: channelId }, token: broadcasterToken },
        { type: 'channel.raid', version: '1', condition: { to_broadcaster_user_id: channelId }, token: broadcasterToken }
    ];
    ...
    for (const sub of subscriptions) {
        try {
            ...
            const response = await fetch('https://api.twitch.tv/helix/eventsub/subscriptions', {
                method: 'POST',
                headers: { 'Client-Id': CLIENT_ID, 'Authorization': `Bearer ${sub.token}`, 'Content-Type': 'application/json' },
            ...
            if (response.status === 202) {
                ...
                failCount++;
                console.error(`[ERROR] EventSub: "${sub.type}" — нет прав (403). Проверьте scope токена`);
            } else if (response.status === 401) {
                failCount++;
                console.error(`[ERROR] EventSub: "${sub.type}" — токен невалиден (401). Обновите токен`);
            } else {
                failCount++;
                console.error(`[ERROR] EventSub: "${sub.type}" (${response.status}):`, result.message || result);
            }
        } catch (error) {
            failCount++;
    ...
    console.log(`${tag} EventSub итого: ${successCount} успешно, ${failCount} ошибок`);
    if (failCount > 0) {
        console.warn('[WARN] Убедитесь, что broadcasterAccessToken имеет все необходимые scope');
    }
...
function reconnectEventSub(reconnectUrl) {
    ...
    if (eventSubKeepaliveTimeout) { clearTimeout(eventSubKeepaliveTimeout); eventSubKeepaliveTimeout = null; }
    if (eventSubReconnectTimeout) { clearTimeout(eventSubReconnectTimeout); eventSubReconnectTimeout = null; }
    ...
    eventSubWs.on('message', async (rawData) => {
        try {
            ...
            if (messageType === 'session_welcome') {
                ...
                console.warn(`[WARN] EventSub: подписка отозвана: ${data.payload.subscription.status}`); 
            }
        } catch (error) { 
...
function disconnectEventSub() {
    if (eventSubWs) { eventSubWs.removeAllListeners(); eventSubWs.close(); eventSubWs = null; }
    if (eventSubKeepaliveTimeout) { clearTimeout(eventSubKeepaliveTimeout); eventSubKeepaliveTimeout = null; }
    if (eventSubReconnectTimeout) { clearTimeout(eventSubReconnectTimeout); eventSubReconnectTimeout = null; }
    eventSubSessionId = null;
    eventSubSubscriptionIds = [];
...
function stopAllTimers() {
    activeTimers.forEach((id, name) => { clearInterval(id); console.log(`[INFO] Таймер остановлен: ${name}`); });
    activeTimers.clear();
}

function startTimer(name, event) {
    if (activeTimers.has(name)) { clearInterval(activeTimers.get(name)); activeTimers.delete(name); }
    ...
    const id = setInterval(async () => {
        try {
            ...
            if (!fresh || !fresh.enabled) { clearInterval(id); activeTimers.delete(name); return; }
            await executePeriodicEvent(name, fresh);
        } catch (e) { console.error(`[ERROR] Событие "${name}":`, e.message || e); }
    }, ms);
...
async function executePeriodicEvent(name, event) {
    const channel = `#${CHANNEL_NAME}`;
    await executeAction(event, channel, BOT_USERNAME, undefined, {}, `Таймер: ${name}`);
}
...
function restartTimers() { console.log('[INFO] Перезапуск таймеров...'); startAllTimers(); }
...
if (tokensValid) {
    ...
    twitchClient.connect().then(() => {
        ...
        const channelIdPromise = fetch(`https://api.twitch.tv/helix/users?login=${CHANNEL_NAME}`, {
            headers: { 'Client-Id': CLIENT_ID, 'Authorization': `Bearer ${OAUTH_TOKEN}` }
        }).then(r => r.json()).then(data => {
            if (data.data?.[0]) { channelId = data.data[0].id; console.log(`[START] ID канала: ${channelId}`); }
            else { console.error('[ERROR] Не удалось получить ID канала'); }
        }).catch(e => console.error('[ERROR] Ошибка получения ID канала:', e.message || e));

        const botIdPromise = fetch(`https://api.twitch.tv/helix/users?login=${BOT_USERNAME}`, {
            headers: { 'Client-Id': CLIENT_ID, 'Authorization': `Bearer ${OAUTH_TOKEN}` }
        }).then(r => r.json()).then(data => {
            if (data.data?.[0]) { botId = data.data[0].id; console.log(`[START] ID бота: ${botId}`); }
            else { console.warn('[WARN] Не удалось получить ID бота'); }
        }).catch(e => console.error('[ERROR] Ошибка получения ID бота:', e.message || e));

        Promise.all([channelIdPromise, botIdPromise]).then(() => {
            completeStartupTask('ids');
            if (channelId) { setTimeout(() => connectEventSub(), 2000); }
            else { console.error('[ERROR] channelId не получен, EventSub не будет подключен'); completeStartupTask('eventsub'); }
        });
    ...
    twitchClient.on('message', async (channel, tags, message, self) => {
        ...
        if (displayName) {
            ...
            if (recentChatters.size > MAX_RECENT) recentChatters.delete(recentChatters.values().next().value);
        ...
        try {
            ...
            if (customRewardId) {
                ...
                console.log(`[INFO] IRC: Награда ${customRewardId} от ${displayName}: "${message}"`);
            ...
            if (banned.found) {
                console.log(`[INFO] Запрещённое слово "${banned.word}" от ${displayName}`);
                if (channelId && botId && tags.id) {
                    try {
                        const resp = await fetch(
                            ...
                            { method: 'DELETE', headers: { 'Client-Id': CLIENT_ID, 'Authorization': `Bearer ${OAUTH_TOKEN}` } }
                        );
                        if (resp.status === 204) {
                            console.log('[INFO] Сообщение удалено');
                            await logDeletedMessage(displayName, message, banned.word, banned.type);
                        }
                    } catch (e) {}
                }
                return;
            }
        } catch (e) { console.error('[ERROR] Ошибка обработки банвордов:', e.message || e); }
        ...
        await executeAction(config, channel, displayName, args[0] || undefined, extraVars, config.name || fullCmd);
    });
} else {
    console.log('[INFO] IRC, EventSub и таймеры не запущены (токены не настроены)');
}
...
const storage = multer.diskStorage({
    ...
    filename: (r, f, cb) => cb(null, Date.now() + '-' + crypto.randomBytes(6).readUInt32LE(0).toString(36) + path.extname(f.originalname))
});
const upload = multer({
    storage,
    fileFilter: (r, f, cb) => {
        const ok = ['image/jpeg','image/png','image/gif','image/webp','image/bmp','video/mp4','video/webm','video/ogg','video/quicktime','audio/mpeg','audio/wav','audio/ogg','audio/mp4','audio/flac'];
...
app.get('/api/auth/callback', async (req, res) => {
    ...
    try {
        ...
        console.log('[OAuth] Токен успешно получен для', isBroadcaster ? 'стримера' : 'бота');
...
app.post('/api/auth/refresh/:type', async (req, res) => {
    ...
    if (success) {
        ...
        res.status(500).json({ success: false, error: 'Не удалось обновить токен' });
    }
});

// ========== ОСТАЛЬНЫЕ API РОУТЫ ==========
app.get('/api/health', (req, res) => { res.set('Cache-Control', 'no-store'); res.json({ ok: true, startTime: SERVER_START_TIME }); });
app.get('/api/config', (req, res) => { try { res.json(loadFullConfig()); } catch (e) { res.status(500).json({ error: 'Ошибка' }); } });
        ...
        if (req.body.periodicEvents) {
            for (const [key, event] of Object.entries(req.body.periodicEvents)) {
                if (event.interval && event.interval < 10) {
                    event.interval = 10;
                    console.log(`[INFO] Интервал события "${key}" скорректирован до 10 секунд`);
                }
            }
        }
        if (await saveFullConfig(req.body)) {
            FULL_CONFIG = req.body;
            updateFromConfig();
            if (tokensValid) restartTimers();
...
app.post('/api/upload', upload.single('file'), (req, res) => {
    ...
    res.json({ success: true, file: { originalName: req.file.originalname, fileName: req.file.filename, path: `/media/${req.file.filename}`, size: req.file.size, mimetype: req.file.mimetype } });
});
...
app.get('/api/media-files', async (req, res) => {
    try {
        ...
        const result = await Promise.all(files.map(async f => {
            ...
            return { name: f, path: `/media/${f}`, size: stat.size, modified: stat.mtime, type };
        }));
...
app.get('/api/network-info', (req, res) => {
    const interfaces = require('os').networkInterfaces();
    const addresses = [];
    for (const name of Object.keys(interfaces)) for (const iface of interfaces[name]) if (iface.family === 'IPv4') addresses.push({ interface: name, address: iface.address, internal: iface.internal });
    const config = loadFullConfig();
    res.json({
        server: { port: CONFIG.port, wsPort: CONFIG.wsPort, host: CONFIG.host },
        addresses,
        overlayUrls: (config.overlays || []).map(o => ({ name: o.name, url: `http://${getLocalIP()}:${CONFIG.port}/overlay/${o.path}` })),
...
app.get('/api/debug/viewers', async (req, res) => {
    try {
        ...
        res.json({ success: true, cache: { age: Date.now() - viewersCacheTime, count: viewersCache.length, viewers: viewersCache.slice(0, 20) }, api: { count: viewers.length, viewers: viewers.slice(0, 20) }, recent: { count: recentChatters.size, chatters: Array.from(recentChatters).slice(0, 20) } });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/viewers/random', async (req, res) => { try { res.json({ success: true, viewer: await getRandomViewer() }); } catch (e) { res.status(500).json({ error: e.message }); } });
app.get('/api/periodic-events', (req, res) => { const events = loadPeriodicEvents(); const result = {}; for (const [n, e] of Object.entries(events)) result[n] = { ...e, timerActive: activeTimers.has(n) }; res.json({ success: true, events: result }); });
app.post('/api/periodic-events/restart', (req, res) => { restartTimers(); res.json({ success: true }); });
app.post('/api/periodic-events/:name/trigger', async (req, res) => { const events = loadPeriodicEvents(); const ev = events[req.params.name]; if (!ev) return res.status(404).json({ error: 'Не найдено' }); await executePeriodicEvent(req.params.name, ev); res.json({ success: true }); });
app.get('/api/overlays/connected', (req, res) => { const connected = []; connectedOverlays.forEach((info, ws) => { if (ws.readyState === WebSocket.OPEN) connected.push(info); }); res.json({ success: true, connected }); });
app.get('/api/rewards/channel', async (req, res) => { try { if (!channelId) return res.status(400).json({ error: 'ID канала не определён' }); const token = BROADCASTER_TOKEN || OAUTH_TOKEN; const response = await fetch(`https://api.twitch.tv/helix/channel_points/custom_rewards?broadcaster_id=${channelId}`, { headers: { 'Client-Id': CLIENT_ID, 'Authorization': `Bearer ${token}` } }); if (!response.ok) { const errText = await response.text(); return res.status(response.status).json({ error: `Twitch API: ${response.status}`, details: errText }); } const data = await response.json(); res.json({ success: true, rewards: (data.data || []).map(r => ({ id: r.id, title: r.title, cost: r.cost, isEnabled: r.is_enabled, isPaused: r.is_paused, requiresInput: r.is_user_input_required, backgroundColor: r.background_color, image: r.image?.url_1x || r.default_image?.url_1x || null })) }); } catch (e) { res.status(500).json({ error: 'Ошибка' }); } });
app.get('/api/eventsub/status', (req, res) => { res.json({ connected: eventSubWs && eventSubWs.readyState === WebSocket.OPEN, sessionId: eventSubSessionId, subscriptions: eventSubSubscriptionIds.length, channelId }); });
app.post('/api/eventsub/reconnect', (req, res) => { reconnectEventSub(); res.json({ success: true, message: 'EventSub переподключается...' }); });
app.get('/api/shoutout/status', (req, res) => { res.json({ success: true, done: Array.from(shoutoutDone), queue: [...shoutoutQueue], cooldownUntil: shoutoutCooldownUntil, cooldownRemaining: Math.max(0, shoutoutCooldownUntil - Date.now()), processing: shoutoutProcessing }); });
app.post('/api/shoutout/reset', (req, res) => { shoutoutDone.clear(); console.log('[INFO] Список выполненных shoutout сброшен'); res.json({ success: true }); });
app.post('/api/shoutout/trigger', async (req, res) => { const { username } = req.body; if (!username) return res.status(400).json({ error: 'Укажите username' }); if (!shoutoutQueue.some(name => name.toLowerCase() === username.toLowerCase())) { shoutoutQueue.push(username); console.log(`[INFO] Ручной shoutout для ${username} добавлен в очередь`); processShoutoutQueue(); } res.json({ success: true, message: `${username} добавлен в очередь shoutout` }); });
app.get('/api/events', (req, res) => { res.json({ success: true, events: loadEvents() }); });
app.post('/api/events/:eventType/test', async (req, res) => { const eventType = req.params.eventType; const testData = { follow: { username: 'TestUser', user: 'TestUser', userId: '12345' }, subscribe: { username: 'TestUser', user: 'TestUser', tier: 'Tier 1', tierRaw: '1000', isGift: 'false' }, resubscribe: { username: 'TestUser', user: 'TestUser', tier: 'Tier 1', tierRaw: '1000', months: '6', streakMonths: '3', message: 'Привет! Уже 6 месяцев!' }, giftSub: { username: 'TestGifter', user: 'TestGifter', tier: 'Tier 1', tierRaw: '1000', total: '5', isAnonymous: 'false' }, raid: { username: 'TestRaider', user: 'TestRaider', viewers: '42', fromUserId: '12345' } }; const data = testData[eventType] || { username: 'TestUser', user: 'TestUser' }; await handleEvent(eventType, { ...data, ...(req.body || {}) }); res.json({ success: true, message: `Тестовое событие "${eventType}" выполнено` }); });
app.get('/api/tokens/status', (req, res) => { res.json({ success: true, valid: tokensValid, missing: tokensValid ? [] : tokenCheck.missing }); });
app.get('/overlay/:overlayPath', (req, res) => { res.set({ 'Cache-Control': 'no-cache, no-store, must-revalidate', 'Pragma': 'no-cache', 'Expires': '0' }); res.sendFile(path.join(__dirname, 'overlay.html')); });
app.get('/overlay', (req, res) => { res.set({ 'Cache-Control': 'no-cache, no-store, must-revalidate', 'Pragma': 'no-cache', 'Expires': '0' }); res.sendFile(path.join(__dirname, 'overlay.html')); });
...
app.listen(CONFIG.port, CONFIG.host, () => {
    ...
    if (config.overlays?.length > 0) {
        ...
        config.overlays.forEach(o => console.log(`[START]   "${o.name}" -> http://${getLocalIP()}:${CONFIG.port}/overlay/${o.path}`));
    } else {
        console.log(`[START] Оверлей: http://${getLocalIP()}:${CONFIG.port}/overlay`);
    }
